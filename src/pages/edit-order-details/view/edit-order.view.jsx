...
import {
  ...
  formatDateForInput,
} from '../../../utils/helper/edit-order-helper.js';
...
export default function EditOrderView({
  ...
  const [topoPersonalizado, setTopoPersonalizado] = useState(
    'Não possui topo personalizado',
  );
  ...
  useEffect(() => {
    if (order) {
      ...
      setTopoPersonalizado(
        order.topoPersonalizado || 'Não possui topo personalizado',
      );
    }
  }, [order]);
  ...
  const handleSubmit = e => {
    ...
    const payload = {
      ...
      horaEntrega,
      topoPersonalizado:
        topoPersonalizado === 'Não possui topo personalizado'
          ? null
          : topoPersonalizado,
    };
    ...
    <Container sx={{ p: 0, maxWidth: 480 }}>
      ...
      <Box component='form' onSubmit={handleSubmit} sx={{ p: 3 }}>
        <Stack spacing={2}>
          ...
          {formErrors.dataTime && (
            <Alert severity='error'>{formErrors.dataTime}</Alert>
          )}
          ...
          <FormControl component='fieldset'>
            ...
            >
              <FormControlLabel
                value='Branca de neve'
                control={<Radio />}
                label='Branca de neve'
              />
              <FormControlLabel
                value='Não possui topo personalizado'
                control={<Radio />}
                label='Não possui topo personalizado'
              />
            </RadioGroup>
          </FormControl>

...
import { CakeDetailsView } from './cake-details.view';
import { api } from '../../../../services/api';

/**
 * Controller da tela de detalhes de um bolo específico
 * Gerencia a busca de dados do item e da imagem do anexo
 */
export function CakeDetailsController() {
      ...
      try {
        ...
        if (location.state?.item) {
          ...
          if (itemData.informacaoBolo?.anexo) {
            ...
            anexoId = itemData.informacaoBolo.anexo.idAnexo || 
            ...
            if (itemData.informacaoBolo.anexo.imagemAnexo && !anexoId) {
              ...
              if (typeof itemData.informacaoBolo.anexo.imagemAnexo === 'string') {
                if (itemData.informacaoBolo.anexo.imagemAnexo.startsWith('data:')) {
                  setImageSrc(itemData.informacaoBolo.anexo.imagemAnexo);
                } else {
                  // Assume que é base64
                  setImageSrc(`data:image/jpeg;base64,${itemData.informacaoBolo.anexo.imagemAnexo}`);
                }
                setLoading(false);
                return;
              }
          ...
          if (!anexoId) {
            anexoId = itemData.informacaoBolo?.fk_anexo || itemData.informacaoBolo?.fkAnexo;
          }
        ...
        if (idPedido && idItemPedido) {
          ...
          if (itemEncontrado.informacaoBolo?.anexo) {
            // Tenta diferentes formatos de ID
            anexoId = itemEncontrado.informacaoBolo.anexo.idAnexo || 
            ...
            if (itemEncontrado.informacaoBolo.anexo.imagemAnexo && !anexoId) {
              // Se vier como base64 string
              if (typeof itemEncontrado.informacaoBolo.anexo.imagemAnexo === 'string') {
                if (itemEncontrado.informacaoBolo.anexo.imagemAnexo.startsWith('data:')) {
                  setImageSrc(itemEncontrado.informacaoBolo.anexo.imagemAnexo);
                } else {
                  // Assume que é base64
                  setImageSrc(`data:image/jpeg;base64,${itemEncontrado.informacaoBolo.anexo.imagemAnexo}`);
                }
                setLoading(false);
                return;
              }
          ...
          // Se não encontrou anexoId, tenta fk_anexo direto no informacaoBolo
          if (!anexoId) {
            anexoId = itemEncontrado.informacaoBolo?.fk_anexo || itemEncontrado.informacaoBolo?.fkAnexo;
          }
  ...
  return (
    <CakeDetailsView
      ...
      item={item}
      imageSrc={imageSrc}
      orderId={idPedido || location.state?.orderId}

...
import {
  ...
  btnViewDetails,
  btnAdvance,
} from '../styles/pending-order.style';
...
export function PendingOrderCard({
  ...
  const getDetailText = (step) => {
    ...
    if (step === 2) {
      ...
      if (qtdTemas > 1) {
        return `Cliente escolheu ${qtdTemas} tema${qtdTemas > 1 ? 's' : ''} diferente${qtdTemas > 1 ? 's' : ''}`;
      } else if (qtdTemas === 1) {
        ...
        return `Pedido com ${qtdIngredientes} ingrediente${qtdIngredientes > 1 ? 's' : ''} personalizado${qtdIngredientes > 1 ? 's' : ''}`;
      }
    ...
    if (step === 3) {
      ...
      return `Aguardando pagamento de 50% do pedido (R$ ${valorParcial})`;
    }
...
{/* icone relogio e mensagem do status do pedido */}
        ...
        <Stack direction="row" spacing={2} sx={{ mt: 2 }}>
          ...
          <Button
            ...
            disabled={loading}
          >
            {loading ? 'Carregando...' : isLastStep ? 'Aceitar' : 'Avançar'}
          </Button>
        </Stack>
      </CardContent>
    </Card>
